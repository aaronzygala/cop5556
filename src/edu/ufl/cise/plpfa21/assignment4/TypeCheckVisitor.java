...
public class TypeCheckVisitor implements ASTVisitor {
	...
	public Object visitIBinaryExpression(IBinaryExpression n, Object arg) throws Exception {
		...
		check(compatibleAssignmentTypes(e0Type, e1Type), n, "Binary expression requires matching types!");
		Kind op = n.getOp();

		if ((op == Kind.EQUALS || op == Kind.NOT_EQUALS || op == Kind.LT || op == Kind.GT)) {
			n.setType(PrimitiveType__.booleanType);
		} else if (op == Kind.PLUS && (e0Type.isList() || e0Type.isInt() || e0Type.isString())) {
			n.setType(e0Type);
		} else if ((op == Kind.MINUS || op == Kind.TIMES || op == Kind.DIV) && e0Type.isInt()) {
			n.setType(PrimitiveType__.intType);
		} else if ((op == Kind.AND || op == Kind.OR) && e0Type.isBoolean()) {
			n.setType(PrimitiveType__.booleanType);
		} else {
			// not a legal case
	...
	public Object visitIReturnStatement(IReturnStatement n, Object arg) throws Exception {
		...
		check(compatibleAssignmentTypes(expressionType, dec.getResultType()), n,"inconsistent return type!");
		return arg;
	}
	...
	public Object visitISwitchStatement(ISwitchStatement n, Object arg) throws Exception {
		IExpression e0 = n.getSwitchExpression();
		IType e0Type = (IType) e0.visit(this,  arg);
		check(e0Type.isInt() || e0Type.isBoolean() || e0Type.isString(), n, "Incompatible types in switch expression!");
		
		List<IExpression> expressions = n.getBranchExpressions();
		List<IBlock> blocks = n.getBlocks();
		int i = 0;
		for(IExpression e1: expressions) {
			IType e1Type = (IType) e1.visit(this, arg);
			check(compatibleAssignmentTypes(e1Type, e0Type), n,"incompatible expression types!");
			check(isConstantExpression(e1), n, "expression is not constant!");
			blocks.get(i).visit(this, arg);
			i++;
		}
		
		IBlock blockDefault = n.getDefaultBlock();
		blockDefault.visit(this,  arg);
		return arg;
	}
